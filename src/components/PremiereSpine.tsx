...
const studioOf = (media: any): string =>
  (media?.studios?.edges ?? []).find((edge: any) => edge?.isMain)?.node?.name ?? '';
...
function PremiereSpine({ media }: { media: any[] }) {
  ...
  const { dated, undated, heading, nextIndex } = useMemo(() => {
    ...
    const first = media?.[0];
    const season = SEASON_LABEL[first?.season as string];
  ...
  const poster = (entry: Dated) => (
    ...
    >
      ...
      <span className="spine__studio">{studioOf(entry.media)}</span>
    </Link>
  );
  ...
  return (
    <section className="spine">
      ...
      <div className="spine__scroller" ref={scroller}>
