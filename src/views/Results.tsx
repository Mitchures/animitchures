 ...
 * page into `state.results` by hand, which meant every other view that could
...
function Results() {
  ...
  return (
    ...
    >
      ...
      {isFirstLoad && (
        <div className="results__container" aria-hidden="true">
          {Array.from({ length: SKELETON_COUNT }, (_, index) => (
            <Skeleton key={index} className="results__skeleton" radius="var(--border-radius)" />
          ))}
        </div>
      )}
      ...
      {!isFirstLoad && !error && hasCriteria && !media.length && (
        <p className="results__empty">{nothingFound()}</p>
      )}
      ...
      {!!media.length && (
        <>
          ...
          {pageInfo?.hasNextPage && (
            <button type="button" className="results__more" onClick={loadMore} disabled={loading}>
